...
weights = torch.load('./weights/yolov7-w6-pose.pt', map_location=device)
model = weights['model']
...
#            cv2.putText(img, 'FPS : {:.2f}'.format(fps_), (200, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 2, cv2.LINE_AA)
            cv2.putText(img, 'FPS : {:.2f}'.format(fps_), (150, 20), cv2.FONT_HERSHEY_PLAIN, 1, (0,255,0), 2, cv2.LINE_AA)
